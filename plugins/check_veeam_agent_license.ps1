YNOPSIS
    ...
    Script to check the license expiration date of Veeam Agent for Windows via Nagios.
.DESCRIPTION
    Simple Nagios check to monitor the expiration date of Veeam Agent for Windows.
.PARAMETER Warn
    Threshold for the warning state.
.PARAMETER Crit
    Threshold for the critical state.
.EXAMPLE
    check_veeam_agent_license.ps1 -w 5 -c 2
    Basic use example.
    
.NOTES
    Name: check_veeam_agent_license
...
$regkey_veeam = (Get-Item 'HKLM:\SOFTWARE\Veeam\Veeam Agent for Microsoft Windows\license').GetValue('Lic1')
...
If ($licdays_veeam -gt $warn) {
    Write-Host "OK! $licdays_veeam days left for Veeam Agent license to expire."
    Exit 0
}
ElseIf ($licdays_veeam -gt $crit) {
    Write-Host "WARNING! $licdays_veeam days left for Veeam Agent license to expire."
    Exit 1
}
Else {
    Write-Host "CRITICAL! $licdays_veeam days left for Veeam Agent license to expire."
    Exit 2
}
